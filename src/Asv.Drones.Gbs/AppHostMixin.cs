...
public static class AppHostMixin
{
    ...
    public static IHostApplicationBuilder AddSystemTimeProvider(this IHostApplicationBuilder builder)
    {
        builder.Services.AddSingleton(TimeProvider.System);
        return builder;
    }

    public static IHostApplicationBuilder AddUserConfig(this IHostApplicationBuilder builder, string fileName)
    {
        builder.Services.AddSingleton<IConfiguration>( provider =>
        {
            var logger = provider.GetRequiredService<ILoggerFactory>().CreateLogger<JsonOneFileConfiguration>();
            return new JsonOneFileConfiguration(fileName, true, TimeSpan.FromMilliseconds(500), true,logger);
        });
        return builder;
    }
}