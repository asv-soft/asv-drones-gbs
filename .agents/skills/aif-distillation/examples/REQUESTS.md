...
- write each child directly under `.agents/skills/<prefix>-<child-scope>/`
...
- no writes to the current agent `.agents/skills`
...
- confirm the resolved target path would stay under the resolved output root (`.agents/skills` by default, or `--path` when present)
